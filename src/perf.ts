...
export function timeThisABAsync<A, B>(func: λ<A, Promise<B>>, name: string, ms = 500): λ<A, Promise<B>> {
  return async (x: A) => {
    const start = performance.now()
...
export function timeThisAABAsync<A, B, C>(func: (x: A, y: B) => Promise<C>, name: string, ms = 500): (x: A, y: B) => Promise<C> {
...
export function timeThisAAAsync<A, B>(func: (x: A, y: B) => Promise<void>, name: string, ms = 500): (x: A, y: B) => Promise<void> {
  return async (x: A, y: B) => {
    const start = performance.now()
    await func(x, y)
    const duration = performance.now() - start
