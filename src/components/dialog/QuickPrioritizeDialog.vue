  ...
  <q-dialog ref="dialogRef" maximized @hide="onDialogHide">
    <q-card class="q-dialog-plugin">
      <q-card-section class="bg-primary text-white text-center">
        ...
        <div class="text-h6">
          Which tasks should come after this task: {{ task.title }}
        </div>
        <q-btn
          class="q-ma-sm"
          size="md"
      ...
      <q-card-section>
        <q-btn
          ...
          color="negative"
          :label="
            layerZero.some((x) => !x.selected) ? 'SELECT ALL' : 'UNSELECT ALL'
          "
          @click="selectAll"
        />
        <q-list class="text-primary">
          ...
          >
            <q-item>
              <q-checkbox
                v-model:model-value="currentTask.selected"
                color="primary"
                keep-color
              />
...
const selectAll = () => {
  if (layerZero.some((x) => !x.selected))
    layerZero.forEach((x) => (x.selected = true))
