<template>
  <div
    class="fixed-center"
    style="width: max(min(100%, 500px), 40%); text-align: center"
  >
    <q-card
    ...
    >
      <q-bar style="background-color: #333333">
        ...
        <q-btn
          ...
          flat
          icon="fa fa-check"
          @click=";(currentTask as Task).toggleCompleted()"
        />
      </q-bar>
...
const layerZero = computed(() => {
  ...
  return useRepo(TaskRepo)
    ...
    .sort(
      (a, b) =>
        b.hard_postreqs.filter(incomplete).length -
        a.hard_postreqs.filter(incomplete).length
    )
})
