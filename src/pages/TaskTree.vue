...
const layerZeroTasks = computed((): SimpleTreeNode<Task>[] => {
  const x = tr.where((x) => x.hard_prereq_ids.length === 0).get().map((x) => ({
    id: x.id ?? -1,
    obj: x,
    label: x.title,
    expandable: x.hard_postreq_ids.length > 0,
    lazy: x.hard_postreq_ids.length > 0,
    key: x.id + '('
  }))
...
const loadPostreqs = (d: details<Task>) => {
  d.node.obj.expanded = true
    const postreqs = tr.with('hard_postreqs').where((x) => d.node.obj.hard_postreq_ids.includes(x.id)).get().map((x): SimpleTreeNode<Task> => {
      return {
        id: x.id ?? -1,
        obj: x,
        label: x.title,
